...
#
# from auto_graphql.views import FlyingGraphQLView
...
#         if isinstance(obj, FlyingGraphQLView):
#             schema = obj.schema
#     client = Client(schema)
#     executed = client.execute(query)
#     assert executed == expected_resp
...
class ViewsTestCase(GraphQLTestCase):
    ...
    question {
      ...
      polls {
        ...
        self.assertResponseNoErrors(response)
        content = json.loads(response.content)
        #  1 1 {1 2}; 2 1 {1 2}; 3 1 {1 2}; 4 2 1; 5 2 1
...
question {
  id
    ...
    def test_query_filter_questions_by_two_m2ms(self):
            response = self.query('''
        query MyQuery {
        filterQuestion(polls: [1], buckets: [1]) {
        id
        }
        }
                ''')
